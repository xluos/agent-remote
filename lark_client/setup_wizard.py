...
# agent_remote 机器人所需的最小权限 scope 集合
# 格式：飞书 OAuth scope（空格分隔）
AGENT_REMOTE_SCOPES = " ".join([
    # 卡片
    "cardkit:card:write",
    # 通讯录
...
# 仅保留 Agent Remote 实际使用的权限（消息收发、卡片、群聊管理、加急通知）
TENANT_SCOPES = [
    # 卡片
    "cardkit:card:write",
    # 通讯录
...
def authorize_app_scopes(app_id: str, app_secret: str,
    ...
    """
    try:
        auth = request_device_authorization(app_id, app_secret, AGENT_REMOTE_SCOPES, brand)
