...
Claude/Codex CLI (PTY)
  ...
  server.py         ← PTY 代理，管理进程/控制权/历史缓存
      │
  Unix Socket (/tmp/agents-remote/<name>.sock)
      │
  ┌───┴────┐
  │        │
...
> `_screen.log` 只在 CLI 传入 `--debug-screen` 时写入：每次 flush 触发后、正式解析 `pyte.Screen` 之前，`server/server.py` 会覆盖输出当前屏幕快照到 `/tmp/agents-remote/<name>_screen.log`，方便比对 blink 标记、行号与布局切分。
...
- **持久化绑定**：`~/.agents-remote/lark_chat_bindings.json`（chat_id → session_name）
...
**调试文件**：
- `/tmp/agents-remote/<name>_messages.log` — ClaudeWindow 快照（每个 block 含 block_id）
- `/tmp/agents-remote/<name>_screen.log` — pyte 屏幕快照（`--debug-screen` 开启时写入）
- `/tmp/agents-remote/<name>_debug.log` — server 调试日志（需设置 `SERVER_LOG_LEVEL=DEBUG`，包含 blink 检测和 flush 耗时）
...
# 飞书客户端管理（需配置 .env）
agents-remote lark start     # 启动（后台守护进程）
agents-remote lark stop      # 停止
agents-remote lark restart   # 重启
agents-remote lark status    # 查看状态和日志
```
...
**需要活跃会话的集成测试：**
```bash
# 先启动会话：agents-remote start test
...
- **飞书配置：** 复制 `.env.example` 为 `~/.agents-remote/.env`，填写 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`
- **Socket 路径：** `/tmp/agents-remote/<name>.sock`，PID 文件同目录
...
- **PID 文件：** `/tmp/agents-remote/lark.pid`
- **状态文件：** `/tmp/agents-remote/lark.status` (JSON 格式，包含启动时间)
- **日志文件：**
  - `~/.agents-remote/lark_client.log` — 正常运行日志（INFO 及以上）
  - `~/.agents-remote/lark_client.debug.log` — 调试日志（需设置 `LARK_LOG_LEVEL=DEBUG`）

启动后客户端在后台运行，通过 `agents-remote lark status` 可查看：
- 进程 PID
- 启动时间（精确到秒）
- 运行时长（格式化为天/小时/分钟）
- 日志文件大小
