...
Agent Remote 是一个双端共享 Claude/Codex CLI 工具。通过 PTY + Unix Socket 架构，支持多个终端客户端和飞书客户端并发连接同一个 Claude 或 Codex 会话，实现协作式 AI 对话。
...
Claude/Codex CLI (PTY)
  ...
  server.py         ← PTY 代理，管理进程/控制权/历史缓存
      │
  Unix Socket (/tmp/agent-remote/<name>.sock)
      │
  ┌───┴────┐
  │        │
...
**核心模块：**
- `agent_remote.py` — CLI 入口，子命令：start / attach / list / kill / lark
...
> `_screen.log` 只在 CLI 传入 `--debug-screen` 时写入：每次 flush 触发后、正式解析 `pyte.Screen` 之前，`server/server.py` 会覆盖输出当前屏幕快照到 `/tmp/agent-remote/<name>_screen.log`，方便比对 blink 标记、行号与布局切分。
...
- **持久化绑定**：`~/.agent-remote/lark_chat_bindings.json`（chat_id → session_name）
...
**调试文件**：
- `/tmp/agent-remote/<name>_messages.log` — ClaudeWindow 快照（每个 block 含 block_id）
- `/tmp/agent-remote/<name>_screen.log` — pyte 屏幕快照（`--debug-screen` 开启时写入）
- `/tmp/agent-remote/<name>_debug.log` — server 调试日志（需设置 `SERVER_LOG_LEVEL=DEBUG`，包含 blink 检测和 flush 耗时）
...
# 启动会话
uv run python3 agent_remote.py start <会话名> [-- claude 参数]
uv run python3 agent_remote.py start mywork

# 连接/管理会话
uv run python3 agent_remote.py attach <会话名>
uv run python3 agent_remote.py list
uv run python3 agent_remote.py kill <会话名>

# 飞书客户端管理（需配置 .env）
uv run python3 agent_remote.py lark start     # 启动（后台守护进程）
uv run python3 agent_remote.py lark stop      # 停止
uv run python3 agent_remote.py lark restart   # 重启
uv run python3 agent_remote.py lark status    # 查看状态和日志
```
...
**需要活跃会话的集成测试：**
```bash
# 先启动会话：uv run python3 agent_remote.py start test
...
- **飞书配置：** 复制 `.env.example` 为 `~/.agent-remote/.env`，填写 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`
- **Socket 路径：** `/tmp/agent-remote/<name>.sock`，PID 文件同目录
...
- **PID 文件：** `/tmp/agent-remote/lark.pid`
- **状态文件：** `/tmp/agent-remote/lark.status` (JSON 格式，包含启动时间)
- **日志文件：**
  - `~/.agent-remote/lark_client.log` — 正常运行日志（INFO 及以上）
  - `~/.agent-remote/lark_client.debug.log` — 调试日志（需设置 `LARK_LOG_LEVEL=DEBUG`）

启动后客户端在后台运行，通过 `uv run python3 agent_remote.py lark status` 可查看：
- 进程 PID
- 启动时间（精确到秒）
- 运行时长（格式化为天/小时/分钟）
- 日志文件大小
