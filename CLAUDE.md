...
# 启动会话
agent-remote start <会话名> [-- claude 参数]
agent-remote start mywork

# 连接/管理会话
agent-remote attach <会话名>
agent-remote list
agent-remote kill <会话名>

# 飞书客户端管理（需配置 .env）
agent-remote lark start     # 启动（后台守护进程）
agent-remote lark stop      # 停止
agent-remote lark restart   # 重启
agent-remote lark status    # 查看状态和日志
```
...
启动后客户端在后台运行，通过 `agent-remote lark status` 可查看：
- 进程 PID
- 启动时间（精确到秒）
- 运行时长（格式化为天/小时/分钟）
- 日志文件大小
