...
pack_npm_package() {
    ...
    if npm pack > "$RESULTS_DIR/pack.log" 2>&1; then
        PACK_FILE=$(ls -t agent-remote-*.tgz 2>/dev/null | head -1)
        VERSION=$(echo "$PACK_FILE" | sed 's/agent-remote-\(.*\)\.tgz/\1/')
...
EOF
    ...
    timeout 20 uv run python3 agent_remote.py lark start > "$RESULTS_DIR/lark_start.log" 2>&1
    local rc=$?
    if [ $rc -eq 124 ]; then
    ...
    timeout 20 uv run python3 agent_remote.py start "$session" \
    ...
    timeout 20 uv run python3 agent_remote.py start "$codex_session" --cli codex \
...
test_basic_commands() {
    ...
    if grep -q "agent_remote.py" "$install_dir/../bin/cla"; then
        log_success "cla 脚本包含 agent_remote.py"
        report "✓ cla 脚本包含 agent_remote.py"
    else
        log_error "cla 脚本缺少 agent_remote.py"
        report "✗ cla 脚本缺少 agent_remote.py"
        return 1
    fi
...
cleanup() {
    ...
    echo -e "  docker exec ${cid} bash -c 'ls -la /home/testuser/test-npm-install/node_modules/agent-remote/'"
    echo ""
