...
collect_python_info() {
    ...
    # 虚拟环境包
    if [ -d "/home/testuser/test-npm-install/node_modules/agent-remote/.venv" ]; then
        echo "--- 虚拟环境包 ---" >> "$DIAG_DIR/python.txt"
        /home/testuser/test-npm-install/node_modules/agent-remote/.venv/bin/pip list 2>&1 >> "$DIAG_DIR/python.txt"
    fi
...
collect_file_structure() {
    ...
    if [ -d "/home/testuser/test-npm-install/node_modules/agent-remote" ]; then
        cd /home/testuser/test-npm-install/node_modules/agent-remote
