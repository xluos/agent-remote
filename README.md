...
> 包名是 `agents-remote`（`agents-remote` 已被他人占用），但安装后的命令仍是 `agents-remote` / `cla` / `cl` / `cx` / `cdx`。
...
> **npm 包名**：`agents-remote`（`agent-remote` 已被他人占用）。安装后的命令仍是 `agent-remote` / `agents-remote` / `cla` / `cl` / `cx` / `cdx`。
