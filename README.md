...
> 包名是 `agents-remote`（`agent-remote` 已被他人占用），但安装后的命令仍是 `agent-remote` / `cla` / `cl` / `cx` / `cdx`。
...
> `Ctrl+Q` 只断开本地终端视图，**不会终止会话**；要彻底结束会话用 `agent-remote kill <会话名>`。

### 飞书客户端

```bash
agent-remote lark start         # 启动（后台运行）
agent-remote lark stop          # 停止
agent-remote lark restart       # 重启
agent-remote lark status        # 查看状态
```
