...
本指南介绍如何使用 Agent Remote 的飞书客户端管理功能。
...
```bash
# 创建 logrotate 配置
cat > /etc/logrotate.d/agent-remote << EOF
/path/to/agent_remote/lark_client.log {
    daily
    rotate 7
    compress
    delaycompress
...
<dict>
    ...
    <array>
        ...
        <string>/path/to/agent_remote/agent_remote.py</string>
        <string>lark</string>
        <string>start</string>
    </array>
    <key>WorkingDirectory</key>
    <string>/path/to/agent_remote</string>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    <true/>
    <key>StandardOutPath</key>
    <string>/path/to/agent_remote/lark_client.log</string>
...
创建 `/etc/systemd/system/agent-remote-lark.service`：

```ini
[Unit]
Description=Agent Remote Lark Client
After=network.target

[Service]
Type=simple
User=your-username
WorkingDirectory=/path/to/agent_remote
ExecStart=/usr/bin/python3 /path/to/agent_remote/agent_remote.py lark start
ExecStop=/usr/bin/python3 /path/to/agent_remote/agent_remote.py lark stop
Restart=on-failure
RestartSec=10
...
sudo systemctl enable agent-remote-lark
sudo systemctl start agent-remote-lark

# 查看状态
sudo systemctl status agent-remote-lark
```
...
2. 提交 Issue：[GitHub Issues](https://github.com/yourusername/agent_remote/issues)
